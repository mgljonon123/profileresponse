...
const StepperSidebar: React.FC<StepperSidebarProps> = ({
  steps,
  currentStep,
  onStart,
}) => {
  return (
    ...
    >
      <div className="flex justify-between items-center w-full px-8 mb-10">
        <button className="bg-gray-200 text-black px-4 py-1 rounded-lg font-semibold text-sm mr-auto">
          Тест эхлэх
        </button>
        ...
        {steps.map((step, idx) => (
          <div key={idx} className="flex flex-col items-center w-full">
            <div className="flex items-center w-full">
              <span
                className={
                  "w-8 h-8 rounded-full border-2 flex items-center justify-center mr-4 transition-all duration-200 border-white bg-white text-black"
                }
              >
                {idx === currentStep ? (
                  <svg
              ...
              <span
                className={`text-lg font-bold ml-2 ${
                  idx === currentStep ? "text-white" : "text-white/80"
                }`}
              >
                {step}
              </span>
