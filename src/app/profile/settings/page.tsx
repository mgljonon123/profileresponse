  ...
  return (
    <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg p-10 mt-8 border border-[#f0f0f5]">
      ...
      <div className="flex gap-10 items-start">
        ...
        <div className="flex flex-col items-center w-64 bg-[#f7f7fa] rounded-2xl p-6 shadow-sm border border-[#f0f0f5]">
          ...
          <div className="text-center">
            ...
            <button className="bg-blue-600 text-white px-6 py-2 rounded-lg shadow hover:bg-blue-700 transition font-semibold">Засах</button>
          </div>
        </div>
      ...
      <div className="mt-10 bg-[#f7f7fa] rounded-2xl p-8 border border-[#f0f0f5] shadow-sm">
        <div className="font-semibold mb-4 text-[#232360] text-lg">My email Address</div>
        <div className="flex flex-col gap-4">
          {emails.map((e, i) => (
            <div key={i} className="flex items-center gap-4 bg-white rounded-lg px-4 py-3 shadow-sm border border-[#e0e0e7]">
              <div className="bg-blue-100 text-blue-600 rounded-full w-8 h-8 flex items-center justify-center font-bold">@</div>
              <div className="font-medium text-[#232360]">{e.email}</div>
              <div className="text-gray-400 text-sm">{e.added}</div>
            </div>
          ))}
          <button onClick={addEmail} className="mt-2 text-blue-600 bg-blue-50 px-5 py-2 rounded-lg font-semibold shadow hover:bg-blue-100 transition w-fit">+ Add Email Address</button>
        </div>
      </div>
    </div>
  );
